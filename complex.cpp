...
double abs(TComplex complex) {
  const auto &im = complex.getIm();
  const auto &re = complex.getRe();
...
std::ostream &operator<<(std::ostream &out, TComplex complex) {
  return out << '(' << complex.real << (complex.imaginary < 0 ? " - " : " + ")
             << (complex.imaginary < 0 ? complex.imaginary * -1
                                       : complex.imaginary)
             << "i)";
}
...
TComplex TComplex::operator-() { return -1 * *this; }
...
TComplex operator*(int lambda, TComplex complex) { return complex * lambda; }
